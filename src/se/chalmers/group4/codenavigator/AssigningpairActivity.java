...
public class AssigningpairActivity extends Activity {
	...
	private void setUpCurrencyMemberListSpinner() {
		...
		adapter = new ArrayAdapter<String>(this,
				...
				listOfAvailableProgrammers);
		adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);

		// Find the all spinners and connect to adapter to it, and define
		// listener
	...
	private class MemberLoadTask extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... v) {
			...
			try {
				...
				// Which team do I belong to? the GHMyself represents the
				// account that's logging into GitHub
				GHMyself myself = githubObject.getMyself();
				if (myself == null)
					Log.d("assigning", "myself is NULL");
				else {
				...
				if (myTeam == null)
					Log.d("assigning", "myTeam is NULL");
				else
					Log.d("assigning", "my team is found: " + myTeam.getName());
				...
				Log.d("error",
						"Member list, Exception during team member list loading : + "
								+ e.getMessage());
	...
	private class MemberListListener implements OnItemSelectedListener {
		...
		public void onItemSelected(AdapterView<?> parent, View view, int pos,
				long id) {
			choosenProgrammerIndex = memberListSpinner
					.getSelectedItemPosition();
			TextView navigator = (TextView) findViewById(R.id.navigator);
			navigator.setText(AssigningpairActivity.NAVIGATOR_LABEL
					+ theNavigator);
			...
			programmer.setText(AssigningpairActivity.PROGRAMMER_LABEL
					+ listOfAvailableProgrammers.get(choosenProgrammerIndex));
